...
var Analytics = (function(){
  ...
  // console: show debug menu via textbox element.
  var logger = main.analytics.console;
  var logTitle = main.analytics.consoleTitle;
  var closeConsoleBtn = main.analytics.closeConsole;
  var openConsoleBtn = main.analytics.openConsole;
  var oldLog;
  function getConsole(msg) {
    if (!msg) {
    console.log = function (message) {
      if (typeof message == 'object') {
        logger.innerHTML += '\n > ' + (JSON.stringify ? JSON.stringify(message) : message);
      } else {
        logger.innerHTML += '\n > ' + message;
      }
    };
  ...
  logger.addEventListener('click', scrollToBottom);
  logger.addEventListener('input', scrollToBottom, false);
  logger.addEventListener('onpropertychange', scrollToBottom);
  logger.addEventListener('change', scrollToBottom);
  
  /*toggle, show, and hide console*/
...
function changeTitle() {
  ...
  if (titleTimeShown === true) {
    document.title  = `${titleMain} | ${songName} by ${songArtist}\n${formatTime(main.seekbar.value)} | ${formatTime(main.audio.duration)}`;
  } else {
    document.title  = `${titleMain} | ${songName} by ${songArtist}`;
  }
}
...
function getSongLengthFromUrl(src) {
...
function setPlayStateOnLoad() {
  console.log('hi')
  var wasPlaying = sessionStorage.getItem('wasPlaying');
  if (sessionStorage.getItem('wasPlaying')) {
    for (let i = 0; i < 10; i++) {
      if (wasPlaying === 'true') {
        main.audio.play();
      }
